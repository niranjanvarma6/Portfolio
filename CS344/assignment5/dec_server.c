...
void decrypt(){
  ...
  //assign the first token to the ciphertext 
  ...
  //first index goes through the key and ciphertext and converts each letter at a time
  for(j = 0; j < size; j++){  
      ...
      for (i = 0; i < 27; i++){
        if (ciphertext[j] == text[i]){
          //assign integer value for convertion
     		  cipher = i;  
          break;          
     	  }  
      }
      //adds an extra just to make sure no values are negative leading to wacky decryption
      if(cipher < k){
        cipher += 27;
      }
      //converting to integer value for plaintext one by one conversion of characters
      plain = (cipher - k) % 27;
...
int main(int argc, char *argv[]){
    ...
    while (1){
        ...
        if (child == 0){ 
            ...
            //Make sure it is the dec_client through its unique "dec" signal
            ...
            // Send a Success message back to the dec_client
            ...
            //Read the dec_client message with the ciphertext
            charsRead = recv(connectionSocket, buffer, 600, 0); 

            //Call to decrypt the freshly recieved ciphertext
            decrypt();


            // Send the plaintext to the enc_client
