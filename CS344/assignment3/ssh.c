...
//this function expands the double dollar symbols into one with a pid
void expand_dollar(char d[2048]){
    int i;
    for (i = 1; i< (strlen(d)+1); i++){ 
        //when the $$ is found
        if (d[i] == '$' && d[i+1] == '$'){
            strcpy(&d[i], &d[i+1]); //copy it twice so both dollar signs are moved back to not be noticed when copied
            strcpy(&d[i], &d[i+1]);
            char pid_s[20]; //create a pid string
            char temp[2000]; //temp to move data around
            sprintf(pid_s, "%d", getpid()); //assign the string to a pid
            memset(temp,0,strlen(temp));
            strncpy(temp, d, i); //copy the the rest of the command excluding the $$
            strcat(temp, pid_s); // concatanate the pid string
            strcat(temp, d + i); // concatanate the command into temp with its index
            strcpy(d, temp); //copy the temp with the commands back into the input
        }
    }
}
...
void execut(char* com_data[], char infile[300], char outfile[300], int* stat_id){
    ...
    //when the process is returned to the parent/caller
        ...
        //execute the file knowing all child processes are finished
        if(background_mode == 0 || background_check == 1){
            pid_t verifypid = waitpid(spawnpid, stat_id, 0);
        }
        //process returned and executed to avoid bad child processes
        else{
