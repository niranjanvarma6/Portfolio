...
{
  struct list* link_list = malloc(sizeof(struct list));
  assert(link_list);
  link_list->head = NULL;
  return link_list;
}
...
{
  struct link* present_link;
  struct link* past_link;
  struct link* future_link;

  present_link = list-> head;
  past_link = NULL;
  future_link = NULL;

  while(present_link != NULL){
    future_link = present_link->next;
    present_link->next = past_link;
    past_link = present_link;
    present_link = future_link;
  }
  list->head = past_link;
