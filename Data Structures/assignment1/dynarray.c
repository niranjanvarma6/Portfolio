...
void dynarray_insert(struct dynarray* da, void* val)
{
    if(da->size >= da->capacity){
        da->capacity *= 2;
        void** stretchArray = malloc(da->capacity * sizeof(void*));
        for(int a = 0; a < da->size; a++){
            stretchArray[a] = da->data[a];
        }
        free(da->data);
        da->data = stretchArray;
    }
    da->size++;
...
{
    da->size--;
    for(int a = idx; a < da->size; a++){
        da->data[a] = da->data[a+1];
    }
}
...
void* dynarray_get(struct dynarray* da, int idx)
{
    if((idx < 0) || (idx >= da->size)){
        printf("Error return index at 0 \n" );
        return da->data[0];
    }
    else{
        return da->data[idx];
